...
При этом сами узлы в json должны формироваться по новому:
* формат узла json: `"node tag name": { "value": "sum of numbers", "child node name 1": "...", ...}`
* значение `value` узла должно быть равно сумме всех чисел которые встретились в этом узле и во всех его дочерних узлах
